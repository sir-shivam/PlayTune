...
export default function Home() {
  ...
  const PlaylistCard1 = ({info , playlistId })=>{
    ...
    return(
<div  className=' w-full h-full border  rounded-2xl m-8 flex  flex-col  items-center  '  onClick={()=>{navigate("/playlist/view/"+ playlistId )}}  >
<div className='r w-[80%] h-[20%] m-2 text-white text-3xl flex justify-center items-center'>{info.name} </div>
<img src={info.thumbnail} className='box1  w-[80%] h-[65%] mt-3 rounded-xl '></img>
</div>
    )
   }
    ...
    <div>
            <div className='full w-screen h-screen bg-[#0f0f0f] flex'>
                ...
                <div className='w-[80vw] h-[100vh]'>
                    ...
                    {
                     myList1.map((item)=>{
                     return  (<PlaylistCard1 key={JSON.stringify(item )} info={item} playlistId={item._id}  /> )
                    })
                    }
